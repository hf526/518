...
print case_path
#搜索测试用例，加载到用例集合
caselist=unittest.defaultTestLoader.discover(case_path,pattern='smokecase.py')
print caselist
#定义测试报告的名字
...
HTMLTestRunnerEN.HTMLTestRunner(fp,title="Autotest  Report",description=u"测试").run(caselist)
#关闭文件
fp.close()
